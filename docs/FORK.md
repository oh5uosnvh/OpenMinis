...
| `data/repository/ProviderRepository.kt` | `autoRefreshModels` 开头加手动策略跳过；`removeEntry` 之后新增 `addEntries` / `removeEntries` 两个批量方法 | 防止后台刷新回灌全部模型；批量增删一次写盘（400 个模型不再是 400 次 saveConfig） |
...
- `ui/settings/mods/ProviderModelsFetchSheet.kt` —— 获取模型面板（厂商分组 + 每行 ＋/－ 即时增删）
- `ui/settings/mods/ProviderCatalogFetcher.kt` —— 只读目录抓取（fetch 与写入分离）+ 批量增删
...
- `ui/settings/mods/ForkModelHealth.kt` —— 测活（并发上限 / 超时 / 可取消），只在「模型」Tab 用
- `ui/settings/mods/ForkSwipeToRemoveRow.kt` —— 左滑露出方形移除按钮。位置由**单个 Animatable**
  驱动（拖动 snapTo、松手 animateTo），开关状态外提到 `ForkSwipeOpenState`
...
上游改了上表**六个文件的同一区域**时才会冲突（`ProviderRepository.kt` 的两个批量方法是
在 `removeEntry` 之后整段新增的，不改任何既有行，实际冲突概率最低）。真冲突了，按 §4 处理。
